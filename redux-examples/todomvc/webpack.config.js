...
var entryPage=path.join(__dirname, config.base + '/js/containers/');
var webpackDefaultConfig = {
     ...
     */
    entry: {
        'react':['react', 'react-dom',path.join(__dirname, config.base + '/js/util/common')],
        'index':entryPage+"index",
        'app':entryPage+"App"
    },
     ...
     */
    output: {
        path: path.join(__dirname,dev=="php"?config.phpBuild:config.build),
        publicPath:dev=="server"?"":config.staticUrl,//判断是否是打包
    ...
    plugins:[
        ...
        new webpack.optimize.CommonsChunkPlugin("react",dev=="test"?"js/react-[hash:8].js":"js/react.js"),
